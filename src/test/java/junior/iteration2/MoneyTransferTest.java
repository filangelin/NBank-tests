...
public class MoneyTransferTest extends MoneyOperationsBaseTest {
    ...
    private static Stream<Arguments> invalidDataForTransfer() {
        return Stream.of(
                ...
                Arguments.of(user1Accounts.getFirst(),user1Accounts.getFirst(), 100, 400, ""),
                //user cannot transfer more than 10000
                Arguments.of(user1Accounts.getFirst(), user2Account, 10000.01, 400, "Transfer amount cannot exceed 10000"),
                //user cannot transfer 0
                Arguments.of(user1Accounts.getFirst(), user2Account, 0, 400, "Transfer amount must be at least 0.01"),
                //user cannot transfer negative amount
                Arguments.of(user1Accounts.getFirst(), user2Account, -100, 400, "Transfer amount must be at least 0.01"),
                //user cannot transfer more than sender balance
                Arguments.of(user1Accounts.getLast(), user2Account, 10000, 400, "insufficient funds or invalid accounts"),
                //user cannot transfer from nonexisting account
                Arguments.of(99999, user2Account, 100, 403, "Unauthorized access to account"),
                //user cannot transfer to nonexisting account
                Arguments.of(user1Accounts.getFirst(), 99999, 100, 400, "insufficient funds or invalid accounts"),
                //user cannot transfer from not own account
                Arguments.of(user2Account, user1Accounts.getFirst(), 100, 403, "Unauthorized access to account")
        );
    }
